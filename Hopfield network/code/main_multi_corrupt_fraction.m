...
N_exp = 100;
fraction_of_corrupted_neurons_values = 0.0:0.1:0.4;

% Initialize
f_error_pattern_mean = zeros(length(N_pattern_range), length(fraction_of_corrupted_neurons_values));
...
% calculate
for f_idx = 1:length(fraction_of_corrupted_neurons_values)
    fraction_of_corrupted_neurons = fraction_of_corrupted_neurons_values(f_idx);
    ...
    for idx = 1:length(N_pattern_range)
        ...
        [f_error_pattern_mean(idx, f_idx), f_error_pattern_SEM(idx, f_idx), ...
         ...
         generate_corrupt_recall(N_neuron, N_pattern, N_exp, fraction_of_corrupted_neurons);
    end
...
legend(arrayfun(@(x) sprintf('Fraction of Corrupted Neurons = %.1f', x), fraction_of_corrupted_neurons_values, 'UniformOutput', false));
